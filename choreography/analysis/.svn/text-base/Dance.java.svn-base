...
public class Dance
{
  ...
  public Dance(int identifier,ReceptiveField[] attend,ReceptiveField[] shun)
  {
    ID = identifier;
  ...
  public float duration(float time_array[])
  {
    return time_array[last_frame] - time_array[first_frame];
  }
  ...
  public LinkedList<Integer> findStatisticalWeirdness() {
    ...
    float better;
    for (int i=0 ; i<quantity.length ; i++) quantity[i] = Float.NaN;
  ...
  public void findCumulativePath(float speed_window,float[] t,float min_travel) {
    quantityIsBias(t,speed_window,min_travel);
    float[] bias = Arrays.copyOf(quantity,quantity.length);
    double cud;
    Style seg;
    Vec2F u = new Vec2F();
    Vec2F v = new Vec2F();
    Vec2F w = new Vec2F();
    int i0,i1,segi = 0;
    for (i0=0; i0<bias.length; i0=i1) {
      while (i0 < bias.length && Float.isNaN(bias[i0])) quantity[i0++] = Float.NaN;
      if (i0 >= bias.length) break;
      i1=i0;
      while (i1 < bias.length && !Float.isNaN(bias[i1])) i1++;
      cud = 0;
      quantity[i0] = (float)cud;
      seg = (segmentation==null) ? null : segmentation[segi = indexToSegment(i0)];
      for (int i=i0+1; i<i1; i++) {
        if (Math.abs(bias[i])<0.5) quantity[i] = (float)cud;
        ...
        if (seg!=null) {
          ...
          w.eq(bearing[i]).eqNorm();
          v.eqMinus(u);
          if (bias[i] > 0) cud += Math.abs(v.dot(w));
...
//    if (!bias_loaded) findDirectionBias(speed_window,t);
  ...
  public void findEndWiggle()
  {
    ...
    if (spine==null)
    {
      for (int i=0 ; i<centroid.length ; i++) quantity[i] = Float.NaN;
      return;
    }
  ...
  // The following two methods should be identical save for data type
  ...
  public void findQxfwOld2(Choreography chore) {
    ...
    float min_travel = (float)Math.max(
      ...
      chore.min_move_bodylen*body_length.average
    );
    findDirectionBias(chore.speed_window,chore.times,min_travel);
  ...
  public void findQxfwOld(Choreography chore) {
    if (quantity==null || quantity.length != area.length) quantity = new float[area.length];
    ...
    for (i=j=k=0 ; i<bias.length ; i=k) {
      ...
      if (j<bias.length) {
        float t1 = chore.times[j+first_frame];
        j--;
        while (j>=0 && t1-chore.times[j+first_frame] < 1.0f) j--;
        j++;
  ...
  public void allUnload() {
    if (quantity==null || quantity.length != centroid.length) quantity = new float[ centroid.length ];
    loaded_jitter = 0.0f;
  ...
  public void quantityIsBias(float[] times,float speed_window,float min_travel) {
    if (loaded_bias.already) return;
    allUnload();
