...
{
  // We update this by hand for now
  public static String CURRENT_VERSION_IDENTIFIER = "Choreography 1.3.0 build 1035";
  ...
  float mm_per_pixel;
  float min_move_mm;
  float min_move_directional;
  float min_move_bodylen;
  float min_time;
  float speed_window;
  ...
  //LinkedList<CustomComputation> plugins;
  ArrayList<ComputationInfo> plugininfo;
  PlugMapper plug_map;
  ...
  Dance.ReceptiveField attend[];
  Dance.ReceptiveField shun[];
  
  // Various things relating to the mechanics of output
  String file_prefix;  // findFiles may change this
  String[] output_names;
  ...
  public class NullPlugin implements CustomComputation {
    ...
    public boolean computeAll(File out_f) throws IOException { throw new IOException("Missing plugin."); }
    public boolean computeDancerSpecial(Dance d,File out_f) throws IOException { throw new IOException("Missing plugin."); }
  ...
  public class PlugMapper {
    ...
    public CustomComputation this_computation[];
    public int this_index[];
    public int output_count;
    public PlugMapper() {
      already = new HashSet<DataMeasure>();
      int n=0;
      if (plugininfo!=null) for (ComputationInfo ci : plugininfo) {
        if (!ci.mapoutput) continue;
        n += Math.max(0 , ci.plugin.quantifierCount());
      }
      this_computation = new CustomComputation[n];
      this_index = new int[n];
      int m=0;
      if (plugininfo!=null) for (ComputationInfo ci : plugininfo) {
        if (!ci.mapoutput) continue;
        CustomComputation cc = ci.plugin;
        for (int i=0 ; i<cc.quantifierCount() ; i++,m++) {
          this_computation[m] = cc;
          this_index[m] = i;
        }
      }
      output_count = 0;
    }
    public void registerOutput(DataSpecifier ds) {
      if (!already.isEmpty() && !already.contains(ds.measure)) { // Different metrics on same output type are okay
        output_count--;
      ...
      ds.plugin = i2p(output_count);
      ds.p_index = i2i(output_count);
      ds.index = output_count;
      output_count++;
    }
    public CustomComputation i2p(int index) {
      return (index>=0 && index<this_index.length) ? this_computation[index] : new NullPlugin();
    }
    public int i2i(int index) {
      return (index>=0 && index<this_index.length) ? this_index[index] : 0;
    }
  }
  ...
  public class ComputationInfo {
    ...
    public ComputationInfo(CustomComputation cc, String[] args, boolean out) {
      plugin = cc; arguments = args; mapoutput = out;
    }
  }
  ...
  public enum DataMeasure {
    AVG,MED,MAX,MIN,Q4,Q1,STD,SEM,VAR,ONE,NUM,JIT;
    ...
    static
    {
      ...
      interpreter.put("last-quartile",Q4);
      interpreter.put("fourth-quartile",Q4);
      interpreter.put("p75",Q4);
      interpreter.put("first-quartile",Q1);
      interpreter.put("p25",Q1);
  ...
  {
    TIME,FNUM,OBID,NUMB,GOOD,AREA,PERS,SPED,ASPD,LENG,RLEN,WIDT,RWID,ASPC,RASP,MIDL,OUTW,KINK,BIAS,PATH,DIRC,CURV,PHAS,VELX,VELY,LOCX,LOCY,ORNT,CRAB,QXFW,CUST,STI1,STI2,STI3,STI4;
  ...
  public class DataPrinter implements Graphs.FunctionData
  {
    ...
    public String getTitle()
    {
      ...
      if (s_data!=null)
      {
        switch (what)
        {
          ...
          case Q1:
          case Q4: pre = "Quartiles of "; break;
    ...
    {
      return (s_data==null || ! (what==DataMeasure.STD || what==DataMeasure.SEM || what==DataMeasure.Q1 || what==DataMeasure.Q4));
    }
    ...
    public float value(int i)
    {
      ...
      if (s_data!=null)
      {
        switch (what)
        {
          ...
          case Q1:  f = (float)s_data[i].first_quartile; break;
          case Q4:  f = (float)s_data[i].last_quartile; break;
    ...
    public String printValue(float one_value) { return String.format(fformat,one_value*mult); }
    public String print(int i)
    {
      if (s_data!=null || f_data!=null) return String.format(fformat,value(i));
      else if (i_data!=null)
      {
    ...
    public String printAvg(int i,int j)
    {
      ...
      if (s_data!=null || f_data!=null) return String.format(fformat,v_tot);
  ...
  public class DataSpecifier
  {
    ...
    public DataMeasure measure;
    public CustomComputation plugin;
    public int p_index;
    public int index;
    public DataSpecifier() { source=null; measure=null; plugin=null; p_index=index=-1; }
    public DataSpecifier (DataSource s,DataMeasure m) { source = s; measure = m; plugin = null; p_index = index = -1; }
  }
  ...
  public void parseInput(String[] args) throws IllegalArgumentException,CustomHelpException
  {
    ...
    Vector<Double> minmovemm_array = new Vector<Double>();
    Vector<Double> minmovebody_array = new Vector<Double>();
    Vector<Double> speedwin_array = new Vector<Double>();
    Vector<Double> mintime_array = new Vector<Double>();
    Vector<Double> minbiased_array = new Vector<Double>();
    ...
    op.addDouble("m","minimum-move-mm").setStorage(minmovemm_array);
    op.addDouble("M","minimum-move-body").setStorage(minmovebody_array);
    op.addDouble("minimum-biased").setStorage(minbiased_array);
    ...
    print_header = op.optionFound("header");
    ...
    if (minmovemm_array.isEmpty() || minmovemm_array.lastElement().floatValue() <= 0.0f) min_move_mm = 0.0f;
    else min_move_mm = minmovemm_array.lastElement().floatValue();
    
    if (minmovebody_array.isEmpty() || minmovebody_array.lastElement().floatValue() <= 0.0f) min_move_bodylen = 0.0f;
    else min_move_bodylen = minmovebody_array.lastElement().floatValue();
    
    if (minbiased_array.isEmpty() || minbiased_array.lastElement().floatValue() < 0.0f) min_move_directional = -1.0f;
    else min_move_directional = minbiased_array.lastElement().floatValue();
    ...
    for (String s : plugin_array)
    {
      ...
      try { loadComputationPlugin(classname,classargs,true); }
  ...
  {
    if (request.equals("all")) request = "ftnNpsSlLwWaAmkbcd1234";
    ...
    if (has_commas)
    {
      ...
      for (String s : columns)
      {
        ...
        has_colon = (s.indexOf(':') != -1);
        if (has_colon)
        {
          String[] ss = s.split(":");
          spec.add( new DataSpecifier( DataSource.interpret(ss[0]) , DataMeasure.interpret(ss[1]) ) );
        }
    ...
    custom = new Statistic[plug_map.output_count][];
    ...
    for (int i=0; i<requirements.length; i++) {
      ...
      if (!found) {
        try {
          ...
          loadComputationPlugin(name,args,false);
        }
        catch (Exception e) {
  ...
  public void loadComputationPlugin(String classname,String arguments[],boolean mapoutput) throws IOException,IllegalArgumentException,CustomHelpException
  {
    if (classname.equalsIgnoreCase("help")) {
      ...
      System.out.println();
      System.out.println("Note: the long form of the output request is -o custom,");
    ...
    plugininfo.add(new ComputationInfo(cc,arguments,mapoutput));
  }
  
  public void printHelpMessage(boolean show_output)
  {
    System.out.println(CURRENT_VERSION_IDENTIFIER);
    ...
    System.out.println("      --header             Write tab-delimited description of each data column");
    ...
    System.out.println("      all -- same as ftnNpsSlLwWaAmkbcd1234");
  ...
  public float minTravel(Dance d) {
    return (float)Math.max( Math.max(min_move_directional/mm_per_pixel,min_move_mm/mm_per_pixel) , min_move_bodylen*d.body_length.average );
  }
  ...
  public void loadData(String out_name) throws LoadDataException
  {
    ...
    else
    {
      ...
      for (i=0 ; i<n_sitters ; i++)
      {
        ...
        refs[i] = new Dance(id,attend,shun);
    ...
    else if (!blob_in_blobs)
    {
      ...
      for (i=0 ; i<n_dancers ; i++)
      {
        ...
        dances[i] = new Dance(id,attend,shun);
      ...
      for (i=0 ; i<n_files ; i++) {
        ...
        while (id_line != null && id_line.length()>0) {
          ...
          one_dance = new Dance(id,attend,shun);
  ...
  public float[] recomputeOnlineStatistics()
  {
    ...
    else
    {
      for (Dance d : dances) {
        if (d==null) continue;
        float min_travel = (float)Math.max( Math.max(min_move_directional/mm_per_pixel,min_move_mm/mm_per_pixel) , min_move_bodylen*d.body_length.average ); 
        d.quantityIsBias(times,speed_window,min_travel);
      }
    ...
    else
    {
      for (Dance d : dances) {
        if (d==null) continue;
        float min_travel = (float)Math.max( Math.max(min_move_directional/mm_per_pixel,min_move_mm/mm_per_pixel) , min_move_bodylen*d.body_length.average );
        d.quantityIsPath(times,speed_window,min_travel);
      }
   ...
   public void recomputeCustomOnlineStatistics(float[] data) {
    ...
    if (!computables.contains(DataSource.CUST)) custom = null;
      ...
      for (int i=0 ; i<custom.length ; i++) {
        ...
        for (Dance d : dances) {
          if (d==null) continue;
          plug_map.i2p(i).computeDancerQuantity(d , plug_map.i2i(i));
  ...
  public double loadDancerWithData(Dance d,DataSpecifier ds)
  {
    ...
    try {
      ...
      switch (ds.source)
      {
        ...
        case BIAS: d.quantityIsBias(times,speed_window,minTravel(d));
                   break;
        case PATH: d.quantityIsPath(times,speed_window,minTravel(d));
        ...
        case CUST: if (d.loaded_custom.length<=ds.index || !d.loaded_custom[ds.index].already) {
                     ds.plugin.computeDancerQuantity(d,ds.p_index);
                     d.quantityAlreadyIsCustom(ds.index,jitty);
                   }
                   break;
      ...
      if (ds.measure != DataMeasure.JIT && (ds.plugin==null || !(ds.plugin instanceof CustomOutputModification))) {
  ...
  public DataPrinter preparePrinting(DataSpecifier ds,float[] t)
  {
    ...
    switch (ds.source)
    {
      ...
      case CUST: dp.setT( ds.plugin.quantifierTitle(ds.p_index) ).setS( custom[ds.index] ).setDig(3); break;
  ...
  public DataPrinter[] writeStatistics(DataSpecifier[] out_data,String out_name,Dance d) throws Dance.DancerFileException,SaveDataException
  {
    ...
    if (out_data.length==0) write_timecourse=false;
    
    if (out_data.length==0 || (!do_trigger && !write_timecourse)) return null;
  ...
  public void showStatistics(DataSpecifier[] out_data)
  {
    ...
    g.run("Graph View - "+CURRENT_VERSION_IDENTIFIER.split("build")[0]);
  }
  ...
  public void showDataMap(DataPrinter[] dps,DataSpecifier[] dss)
  {
    ...
    dmv = new DataMapVisualizer(dm , zoom_factors , 4 , DataMapper.MINIMAP_SIZE , 0.5 , true , 1,1,2 , "" , new Vec2D(0,0), "Map View - "+CURRENT_VERSION_IDENTIFIER.split("build")[0]);
      ...
      for (ComputationInfo ci : chore.plugininfo) {
        ...
        try {
          ...
          for (Dance d : chore.dances) {
            if (d==null) continue;
            ci.plugin.computeDancerSpecial( d ,
    ...
    if (chore.output_names!=null)
    {
      ...
      if ( (chore.interactive_mode || chore.view_graph || chore.view_datamap))
      {
        if (chore.view_graph)
        {
          ...
          else for (int i=0 ; i<chore.output_names.length ; i++) chore.showStatistics(chore.output_requests[i]);
        }
        if (chore.view_datamap)
        {
          chore.showDataMap(dps,dss);
        }
      }
    }
  }
