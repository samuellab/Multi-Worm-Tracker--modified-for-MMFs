...
public class Flux implements CustomOutputModification {
  ...
  Shape parseShape(String s) throws IllegalArgumentException {
    ...
    Vec2F r = null;
    try {
      c = new Vec2F( Float.parseFloat(ss[1]) , Float.parseFloat(ss[2]) );
      r = new Vec2F( Float.parseFloat(ss[3]) , Float.parseFloat(ss[4]) );
    }
  ...
  void printHelp() {
    ...
    System.out.println("  Coordinates are x,y,a,b where a and b are radii for x and y, in pixels");
    ...
    System.out.println("  Flags govern how the regions are used; flux is the default:");
    System.out.println("    count outputs 0 when an object is out of a region, 1 when in");
    System.out.println("    flux outputs 1 when an object pass into a region, -1 when it passes out");
    ...
    System.out.println("    region numbers start at 0 and are in the order they are specified");
  ...
  ArrayList<Event> findDancerEvents(Dance d) {
    ...
    if ( inside_one(d.centroid[0],shapes) ) ale.add( new Event(chore.times[d.first_frame],2,d.ID,d.centroid[0]) );
    if (d.segmentation==null) {
      for (int i=1; i<d.centroid.length; i++) {
        ...
        if ( j!=0 ) ale.add( new Event(0.5f*(chore.times[i+d.first_frame]+chore.times[i-1+d.first_frame]),j,d.ID,v) );
      }
    }
    else {
      for (int i=0; i<d.segmentation.length; i++) {
        ...
        for (int k=0; k<n; k++) {
          ...
          if (j!=0) {
            float t = 0.5f*(chore.times[i1+d.first_frame] + chore.times[i0+d.first_frame]);
        ...
        if (i+1<d.segmentation.length && d.segmentation[i].i1 < d.segmentation[i+1].i0) {
          ...
          if (j!=0) {
            float t = 0.5f*(chore.times[d.segmentation[i+1].i0+d.first_frame] + chore.times[d.segmentation[i].i1+d.first_frame]);
    ...
    if ( inside_one(d.centroid[d.centroid.length-1],shapes) ) ale.add( new Event(Math.nextUp(chore.times[d.first_frame+d.centroid.length-1]),-2,d.ID,d.centroid[d.centroid.length-1]) );
    return ale;
  }

  public boolean computeAll(File f) throws IOException {
  ...
  public boolean computeDancerSpecial(Dance d, File f) { return false; }
  ...
  public void computeDancerQuantity(Dance d, int which) throws IllegalArgumentException {
    ...
    for (int i=0; i<d.area.length; i++) {
      float t = chore.times[i+d.first_frame];
  ...
  public boolean modifyMapDisplay(Vec2D position, Vec2I dimensions, double pixelSize, BufferedImage buffer, DataMapper dm) { return false; }
}
